...
def read_gml_mt(path):
  """
    read from multi-task .gml file, which has the following format:
      line 0:     N         ;  N is the number of documents.
      line 1...N: M NL L_1 L_2 ... L_NL w_1 w_2 w_3 ... w_M ;
      NL is the #labels; L are the labels of the document, w_1, ..., w_M are words.
  ...
  for line in lines[1:]:
    ...
    labels = [int(label) for label in line[2:2 + num_label]]
    for label in labels:
        label_set.add(label)

    doc = [int(token) for token in line[2 + num_label:]]
    map(lambda w : word_set.add(w), doc)

    all_labels.append(labels)
    docs.append(doc)

  return (docs, all_labels, word_set, label_set)
...
class OnlineMultitaskGibbsMedLDA:
  def __init__(me, num_topic, labels, words, alpha = 0.5, beta = 0.45, c = 1, l = 164, v = 1,
              I = 1, J = 3, stepsize = 1):
    """
    initialize the online Gibbs MedLDA model.
    ...
    Examples
    _____________
    >>> medlda = OnlineMultitaskGibbsMedLDA(5, [0, 1], ["athesim", "god", "beings"])
    >>> medlda = OnlineMultitaskGibbsMedLDA(5, 2, 3)
    ...
    me.medlda = paMedLDAgibbsMT(config)
  ...
  def __check__(me, docs, labels):
    ...
    new_labels = list()
    ind = []
    for (di, (doc, doc_labels)) in enumerate(zip(docs, labels)):
      new_doc = list()
      for w in doc:
        if not me.words.has_key(w):
          warning_word = True
      ...
      if len(new_doc) > 0:
        ...
        for label in doc_labels:
            if not me.labels.has_key(label):
                warning_label = True
            new_doc_labels.append(me.labels[label])
  ...
  def train_with_dataset(me, docs, labels, batchsize):
    ...
    while len(allind) > 0:
      ...
      allind -= set(ind)
      batch_doc = [docs[i] for i in ind]
      batch_labels = [labels[i] for i in ind]
      me.train(batch_doc, batch_labels)
  ...
  def train_with_gml(me, path, batchsize):
    ...
    batchsize: int
      the mini-batch size
    """
    (docs, labels, _, _) = read_gml_mt(path)
    me.train_with_dataset(docs, labels, batchsize)
  ...
  def infer(me, docs, labels = [], num_sample = 100, point_estimate = False):
    ...
    if labels == []:
      ...
      return (predict_labels, ind, me.medlda.testF1())
