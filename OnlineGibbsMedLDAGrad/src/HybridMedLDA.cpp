...
HybridMedLDA::HybridMedLDA(Corpus* corpus, int category) {
	...
	v									= 1;
	c									= 1;
	rate                                = [](int t) -> double{return 10/(1+t); };
	lets_commit							= false;
	lets_batch							= false;
	lets_multic							= false;
	mode_additive						= false;
...
void HybridMedLDA::init() {
	...
	/* stat init*/
	Cdk_test = new double*[testData->D];				
	for( int i = 0; i < testData->D; i++) {
	...
	Zbar_test = new double*[testData->D];
...
void HybridMedLDA::draw_Z_test(Sample* sample, SampleZ* prevZ, int i, CorpusData* dt) {
	...
	for( int j = 0; j < W[i]; j++) {
		...
		double cum = 0;
		for( int k = 0; k < K; k++) {
			weights[k] = cum+(Cdk_test[i][k]+alpha0)*(beta0+gamma[k][t]+Ckt_test[k][t])/(beta0*T+gammasum[k]+Ckt_test_sum[k]);
			cum = weights[k];
			if( isnan(weights[k])) {
				debug( "error: Z weights nan.\n");
			}
	...
	if( prevZ->Zbar == 0)
		prevZ->Zbar = new double*[testData->D];
	if( prevZ->Zbar[i] == 0)
...
void HybridMedLDA::normalize_Phi_Eta(int N, bool remove) {
	...
	cout << "rate " << rate(num_observed) << endl;
	double r = rate(num_observed);
	for( int k = 0; k < K; k++) {
		if(remove) eta_pmean[k] -= r*prev_eta_pmean[k];
		prev_eta_pmean[k] = stat_pmean[k]*data->D/(double)batchSize/(double)N;
		eta_pmean[k] += r*prev_eta_pmean[k];
	}
	for( int k1 = 0; k1 < K; k1++) {
		for( int k2 = 0; k2 < K; k2++) {
			if(remove) eta_icov[k1][k2] -= r*prev_eta_icov[k1][k2];
			prev_eta_icov[k1][k2] = stat_icov[k1][k2]*data->D/(double)batchSize/(double)N;
			eta_icov[k1][k2] += r*prev_eta_icov[k1][k2];
		}
	}
	// %%%%%%%%%%%% update phi, which is dirichlet distribution.
	if(remove) {
		for(int stat_i = 0; stat_i <= prev_gamma_list_end; stat_i++) {
			...
			gamma[k][t] -= r*prev_gamma[k][t];
			gammasum[k] -= r*prev_gamma[k][t];
		}
	}
	prev_gamma_list_end = -1;
...
double HybridMedLDA::train(int num_iter) {
	...
	// sampling.
	int burnin;
	for( burnin = 0; burnin < num_iter; burnin++, batchIdx += batchSize, num_observed += batchSize) {
		/* training */
...
double HybridMedLDA::inference( CorpusData* testData, int num_test_sample, int category) {
...
//		for( int t = 0; t < T; t++) sample->phi[k][t] /= (beta0*T+gammasum[k]);  // normalize.
	...
	for( int it = 0; it < max_gibbs_iter; it++) {
		for( int d = 0; d < testData->D; d++) {
			draw_Z_test(sample, iZ_test, d, testData);
		}
...
//		printf( "%lf %d\n", discriFunc, testData->y[i]);
		int label;
		if( category == -1) label = testData->y[i];
		else label = testData->y[i] == category ? 1 : -1;
		if( testData->py[i] == label) acc++;
	}
	...
	delete[] Ckt_test_sum;
	delete[] eta_lowertriangle;
	return (double)acc/(double)testData->D;
}
