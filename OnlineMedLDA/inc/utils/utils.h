...
template<class T> 
static T** array2d(int d1, int d2) {
	T** res = new T*[d1];
	for(int i = 0; i < d1; i++) {
		res[i] = new T[d2];
...
template<class T>
static void array2d_del(T** array, int d1) {
	if(array) {
		for(int i = 0; i < d1; i++) {
...
template<class T>
static void array1d_del(T* array) {
	delete[] array;
}
...
// template<class type>
// static void shuffleArray(type *idx, int D, int iteration = 1000) {
// 	objcokus cokus;
// 	cokus.seedMT(time(NULL));
// 	for( int it = 0; it < iteration; it++) {
// 		int x = cokus.randomMT()%D, y = cokus.randomMT()%D;
// 		type temp = idx[x];
